...
- `relevance` — `rankEvents()` dari `recommendation.ts` (personalized jika
  `profile` ada, cold-start jika `profile` null/belum lengkap).
...
lolos. Implementasi Supabase menulis `reviewed_by`, `reviewed_at`, dan
...
| Aksi | Field form | Sukses | Gagal |
|---|---|---|---|
| `toggleSaveEventAction` | `eventId`, `returnTo` | redirect ke `returnTo` | redirect ke `/login?next=...` jika belum login |
| `updateTrackerStatusAction` | `eventId`, `status`, `notes?`, `returnTo` | redirect ke `returnTo` | redirect ke `returnTo?error=invalid_status` |
| `removeTrackerAction` | `eventId`, `returnTo` | redirect ke `returnTo` | redirect ke `/login` jika belum login |

Otorisasi diperiksa di dalam masing-masing action via `getSessionUser()` / `requireUser()`.
Efek samping sukses: `revalidatePath('/tracker')`, `revalidatePath('/events')`, `revalidatePath('/')`.
...
| Aksi | Field form | Sukses | Gagal |
|---|---|---|---|
| `createTeamAction` | `eventId`, `title`, `slotsNeeded`, `description?`, `returnTo` | redirect ke `/teams/<id>` | redirect ke `returnTo?error=<pesan>` |
| `joinTeamAction` | `teamId`, `returnTo` | redirect ke `returnTo` | `returnTo?error=Tim ini sudah penuh.` |
| `leaveTeamAction` | `teamId`, `returnTo` | redirect ke `returnTo` | ketua ditolak (harus membubarkan tim) |
| `removeTeamMemberAction` | `teamId`, `memberId`, `returnTo` | redirect ke `returnTo` | `FORBIDDEN` kalau bukan ketua |
| `deleteTeamAction` | `teamId`, `returnTo` | redirect ke `/teams` | `FORBIDDEN` kalau bukan ketua |
...
Hanya pesan `AppError` yang diteruskan ke query string `?error=` — pesan itu
memang ditulis untuk dibaca pengguna. Error lain diganti kalimat generik
supaya detail driver Postgres tidak bocor ke URL.
