...
export default async function TeamDetailPage({
  ...
  const errorMessage = typeof query.error === 'string' ? query.error.slice(0, 200) : null;

  const [user, repository] = await Promise.all([getSessionUser(), getEventRepository()]);
  const team = await repository.getTeamById(id);
  if (!team) notFound();
        ...
        <article>
          ...
          <section aria-labelledby="anggota" className="mt-8">
            ...
            <p className="mt-1 text-sm text-ink-muted">
              {team.members.length} dari {team.slotsNeeded} orang
            </p>
