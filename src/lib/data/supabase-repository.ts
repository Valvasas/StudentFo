...
// rankEvents() (§6) berjalan di memori Node, bukan di SQL — hanya sejumlah
// kandidat TERBARU ini yang ditarik dan diberi skor per request, bukan
// seluruh tabel. Cukup jauh di atas skala Phase 1/2 (§8: ~100 event aktif,
// lalu 500 DAU). Begitu jumlah event APPROVED mendekati angka ini, pindahkan
// skoring ke fungsi SQL/RPC.
...
export class SupabaseEventRepository implements EventRepository {
  async listEvents(query: EventQuery): Promise<Paginated<EventSummary>> {
    ...
    // 'deadline' dan 'newest' murni paginasi di database — tidak ada baris
    // yang perlu ditarik ke memori Node untuk diberi skor.
    ...
    // 'relevance' (default): rankEvents() harus melihat kandidat SEBELUM
    // dipotong per halaman, bukan sesudahnya — memberi skor pada satu
    // halaman yang sudah terpotong hanya bisa mengurutkan ulang di dalam
    // halaman itu sendiri, tidak pernah menaikkan event relevan dari
    // halaman berikutnya. `query.profile` diteruskan apa adanya (termasuk
    // null/undefined untuk pengunjung anonim) — rankEvents() sendiri yang
    // memilih jalur cold-start (recency + popularitas) saat profil kosong,
    // sama seperti MemoryEventRepository.sortSummaries().
    ...
    const total = count ?? data.length;
    const ranked = rankEvents(data.map(toSummary), query.profile ?? null, new Date());
    ...
    return {
      ...
      page,
      pageSize,
      // Dibatasi ke jendela kandidat yang benar-benar diberi skor supaya
      // tidak menjanjikan halaman lebih jauh yang pasti kosong begitu
      // jumlah event APPROVED melewati RANKING_CANDIDATE_LIMIT.
      totalPages:
        total > RANKING_CANDIDATE_LIMIT
