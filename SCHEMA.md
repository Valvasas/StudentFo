...
Batas jumlah anggota **tidak** ditegakkan database (tidak ada CHECK yang
bisa menghitung baris di tabel lain); lihat catatan balapan di
`SupabaseEventRepository.joinTeam()`.

### `ugc_submissions` (Phase 3 — belum ada UI)
...
| `public.expire_past_events()` | Jalankan via cron harian: `APPROVED` + primary deadline lewat → `EXPIRED`. **Tidak menghapus baris.** |
...
| notifications | pemilik | UPDATE pemilik (mark-as-read); INSERT hanya service_role |
| ugc_submissions | tidak ada SELECT publik (privasi email) | INSERT publik `WHERE status='PENDING'` |
| teams | publik read | pemilik (`created_by`) atau admin |
| team_members | authenticated read | self-join INSERT, self/owner DELETE |
...
`sanitizeSearchQuery()` (`src/lib/data/supabase-repository.ts`) sebelum
`websearch_to_tsquery`.

## Cron yang harus dijadwalkan manual (tidak otomatis)

```sql
SELECT public.expire_past_events();             -- jadwalkan harian
SELECT public.create_deadline_notifications();  -- jadwalkan harian, pagi WIB
```

`create_deadline_notifications()` sudah punya penjadwal di repo:
`.github/workflows/deadline-notifications.yml` (butuh GitHub Secrets
`SUPABASE_URL` dan `SUPABASE_SERVICE_ROLE_KEY`). `expire_past_events()`
belum — lihat `TASKS.md` § Pipeline & DevOps.
